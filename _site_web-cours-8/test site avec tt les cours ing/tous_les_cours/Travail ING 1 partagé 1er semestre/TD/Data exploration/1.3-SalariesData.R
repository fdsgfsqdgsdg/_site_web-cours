...
median(salaries$Salaire) # Mediane = 23, c-a-dire 50% des salari?s ont un salaire inferieur ? 23
# La diff?rence entre salaire moyen et m?dian est due aux tr?s gros salaires des cadres sup?rieurs.
...
# Le quantile 0%=18, c a dire il ya 0% des employ?s ou aucun employ? a un salaire inferieur a 18, c'est le minim de salaire alors
#Q1 =21, il ya 25% des employ?s qui ont un salaire inferieure ou egale ? 21
...
# quantiles donn?s par le commande quantile(salaries$Salaire)
...
#Interpretation decile : D1=x, c'a dire il ya 10% des employ?s qui ont un salaire inferrieur a x'
...
D1=quantile(salaries$Salaire,prob=c(0.8)) # pour clculer premier decile tout seul

#Interpretation decile : D1=18, c'a dire il ya 10% des employ?s qui ont un salaire inferrieur a 18

# Ou on peut faire les deux ensembles 

#D9=55.1, c'a dire il ya 90% des employ?s qui ont un salaire inferrieur a 55.1

# Ou on peut faire les deux ensembles 

############################################################################################

# 3- Les dirigeants metteraient en avant les quartiles qui masquent les ?carts de salaire,
#alors que les salari?s parleraient des deciles.
...
# Representation adapt?es : les deux variables sont de types quali, on fait diag en barre et diag circulaire
# On peut les tracer avec les effectifs ou avec les frequences
# Avec les effectifs, on utilise les effectifs calcul?s avec fonction (table)
...
# Avec les frequences, on utilise les frequences calcul?s qu'on a deja calcul? 
...
#ylab = pour nommer les axes des ordonn?s
...
# Comme on a trop des valeurs dans chaque serie, alors les representations par un diagramme en baton est pas adapt?s
# alors, il faut regrouper les donn?es par des classes .
...
#Comparer moyenne obtenu pour donn?es regroup? et donn?es initiale

mean(salaries$Salaire) # Moyenne de la variable initiale avant regrouper les donnes

# On peut remarquer qu'on a pas meme moyenne, c'est ? dire on a de perte d'information.
